...
- **Streams** are the main way of encoding context for events. They act as "folders" in which data is stored. They are organized in a tree structure 
...
- "Smartwatch" contains the collected data from the smartwatch, in particular the geolocation of the user - sub-stream "position" - and the "energy", that can be either the intake of energy of the user or the burnt energy (events of type `energy/cal`).
