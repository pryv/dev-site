...
1. **Choose a base storage engine.** PostgreSQL 14+ (recommended for multi-tenant deployments and cross-user analytics) or SQLite (full alternative — per-user file layout, single-host deployments, GDPR-Art.17 delete-by-unlink semantics). The Docker image bundles `rqlited` (platform DB) and SQLite, but **not** PostgreSQL — for PG, run the database as a sidecar container on a shared network (Docker) or point the config at your existing instance (native). A SQLite-only deployment needs no external database service.
2. `docker pull pryvio/open-pryv.io` (or clone the repo and `just setup-dev-env && just install` for a native install).
3. Write a minimal `override-config.yml` — start from the template in [INSTALL — Minimal production config](https://github.com/pryv/open-pryv.io/blob/master/INSTALL.md#minimal-production-config). **Use literal paths**, not `${ENV_VAR}` placeholders — env-var expansion in config is not supported and the core will refuse to start if it sees any unresolved `${...}` value. Required `storages.engines.*` keys: the chosen base engine block (`postgresql` if you picked PG), plus `filesystem.{attachmentsDirPath,previewsDirPath}`, `sqlite.path`, and `rqlite.{url,raftPort,dataDir}`. All are required at boot.
...
**Upgrading from v1?** If you already run Pryv.io 1.x and want to move its user data into a fresh v2 install, use the [`dev-migrate-v1-v2`](https://github.com/pryv/dev-migrate-v1-v2) toolkit — it exports from a v1 MongoDB and writes a v2-compatible backup that `bin/backup.js --restore` can import into a v2 PostgreSQL or SQLite install. See the toolkit's README for the current source/target matrix.
